...
import { type Insets, type ScrollViewProps, StyleSheet } from "react-native";
...
const calculateKeyboardInset = (height: number, safeAreaInsetBottom: number) => {
    "worklet";
    // Subtract safe area from keyboard height since Android may report keyboard height including safe area.
...
export const KeyboardAvoidingLegendList = (forwardRef as TypedForwardRef)(function KeyboardAvoidingLegendList<ItemT>(
    ...
    // Track keyboard open state to ignore spurious events
    ...
    const calculateTopItemInset = useCallback(() => {
        ...
        if (currentInset !== newTopItemInset) {
            ...
            // so report only the keyboard inset (would be double-counted otherwise)
    ...
    const handleMetricsChange = useCallback(
        ...
        [onMetricsChangeProp, updateScrollMetrics, topItemIndex, scheduleTopItemRecalc],
    );
    ...
    useEffect(() => {
        scheduleTopItemRecalc();
    }, [topItemIndex, scheduleTopItemRecalc]);
    ...
    // Android ignores contentInset, so keyboard spacing is handled via marginBottom in animated style
    ...
    const contentContainerStyle = useMemo(() => {
        ...
        if (topItemInsetState > 0) {
            styles.push({ paddingBottom: topItemInsetState });
        }

        return styles.length > 0 ? styles : undefined;
    }, [contentContainerStyleProp, topItemInsetState]);
